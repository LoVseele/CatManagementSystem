...
const scoresSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchScoresByUser.pending, (state) => { state.loading = true; state.error = null })
      .addCase(fetchScoresByUser.fulfilled, (state, action: PayloadAction<Score[]>) => { state.loading = false; state.items = action.payload })
      .addCase(fetchScoresByUser.rejected, (state, action) => { state.loading = false; state.error = action.error.message || '加载评分失败' })
      .addCase(addScore.fulfilled, (state, action: PayloadAction<Score>) => { state.items.push(action.payload) })
      .addCase(editScore.fulfilled, (state, action: PayloadAction<Score>) => {
        const idx = state.items.findIndex(s => s.id === action.payload.id)
        if (idx !== -1) state.items[idx] = action.payload
      })
      .addCase(deleteScore.fulfilled, (state, action: PayloadAction<number>) => {
